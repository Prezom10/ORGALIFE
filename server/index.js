...
const sendTelegramNotification = async (order) => {
  try {
    ...
    if (!token || !chatId) {
      console.log('[Telegram] Missing credentials - Token:', token ? 'Present' : 'Missing', 'ChatId:', chatId ? 'Present' : 'Missing');
      return;
    }

    console.log('[Telegram] Sending notification for order #' + order.id.slice(-4));

    // Format Items List with quantities
    const itemsList = order.items.map(i => {
      const qty = i.quantity || 1;
      return `• ${i.name} ${qty > 1 ? `(x${qty})` : ''} - ৳${i.price}${qty > 1 ? ` = ৳${i.price * qty}` : ''}`;
    }).join('\n');
